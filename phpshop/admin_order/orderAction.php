...
//三、获取action参数的值，并做对应的操作
	switch($_GET["action"]){
		...
		case "update": //修改
			...
			//2. 数据验证
			if(empty($orderMoney)){
				die(errorTip("订单金额必须填写", "editOrder.php?id={$id}"));
			}
			
			if(empty($consignee)){
				die(errorTip("收货人不能为空", "editOrder.php?id={$id}"));
			}
			
			if(empty($address)){
				die(errorTip("收货地址不能为空", "editOrder.php?id={$id}"));
			}
			
			if(empty($phone)){
				die(errorTip("收货手机号不能为空", "editOrder.php?id={$id}"));
			}
			...
			echo "<br/> <a href='orderList.php'>返回订单列表<a>";
