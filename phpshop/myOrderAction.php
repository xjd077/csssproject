	...
	switch($_GET["action"]){
		case "add": //添加
			...
			if($result && mysql_num_rows($result)>0){
				...
				alertMes('商品不存在', 'shop.php');
			}
			if($orderMoney <= 0){
				alertMes('订单金额需要大于0', 'shop.php');
			}
		...
		case "update": //修改
			...
			//2. 数据验证
			if(empty($orderMoney)){
				alertMes("订单金额必填", "editOrder.php?id={$id}");
			}
			
			if(empty($consignee)){
				alertMes("收货人必填", "editOrder.php?id={$id}");
			}
			
			if(empty($address)){
				alertMes("收货地址必填", "editOrder.php?id={$id}");
			}
			
			if(empty($phone)){
				alertMes("收货人手机号必填", "editOrder.php?id={$id}");
			}
			...
			if(mysql_affected_rows()>0){
				alertMes("修改成功", "myOrderList.php");
			}else{
				//echo "修改失败".mysql_error();
				alertMes("修改失败", "myOrderList.php");
			}
