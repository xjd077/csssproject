...
//三、获取action参数的值，并做对应的操作
	switch($_GET["action"]){
		case "add": //添加
			...
			//2. 验证
			if(empty($username)){
				die(errorTip("用户名不能为空", "addUser.php"));
			}
			if(empty($password)){
				die(errorTip("密码不能为空", "addUser.php"));
			}
			...
			if($result){
				...
				$num=$res['total'];
				if($num > 0){
					die(errorTip("用户名已经存在", "addUser.php"));
				}
			}
		...
		case "update": //修改
			...
			//2. 数据验证
			if(empty($username)){
				die(errorTip("用户名不能为空", "editUser.php?id={$id}"));
			}
			...
			if($result){
				...
				$num=$res['total'];
				if($num > 0){
					die(errorTip("用户名已经存在", "editUser.php?id={$id}"));
				}
			}
		...
		case "resetPwd":
			...
			//1. 验证
			if(empty($password)){
				die(errorTip("密码不能为空", "resetUserPwd.php?id={$id}"));
			}
			if($password != $repassword){
				die(errorTip("两个密码不一致", "resetUserPwd.php?id={$id}"));
			}
			//2. 执行修改
