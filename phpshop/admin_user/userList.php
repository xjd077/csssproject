            ...
            <div class="card rounded-0">
                <form action="<?php echo $_SERVER['PHP_SELF'];?>">
                    <div class="input-group mb-1">
                        <input type="text" class="form-control" name="keyword" value="<?php if(isset($_GET['keyword'])) echo $_GET['keyword'];?>" placeholder="搜索客户名...">
				...
				//4. 解析商品信息（解析结果集）
					while($row = mysql_fetch_assoc($result)){
						...
						if($row['username'] != "admin"){
							...
							<a href='userAction.php?action=del&id={$row['id']}' class='button'>删除</a>
							<a href='editUser.php?id={$row['id']}' class='button'>修改</a>
							<a href='resetUserPwd.php?id={$row['id']}' class='button'>重置密码</a>
							</td>";
						}else{
							echo "<td class='txt_center'>
							<a class='button' onclick='cantDel();'>删除</a>
							<a href='editUser.php?id={$row['id']}' class='button'>修改</a>
							<a href='resetUserPwd.php?id={$row['id']}' class='button'>重置密码</a>
							</td>";
						}
